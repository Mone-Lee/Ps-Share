    ...
    <div>
        ...
        <table class="report-list">
            ...
            <template v-for="(Report, index) in Reports">
                <tr>
                    <td class="name" :title="Report.share_abstract">
                        {{Report.share_abstract}}
                    </td>
                    ...
                    <td class="operate">
                        <button @click="more(Report.share_id)" type="button">查看</button>
                        <button v-if="Report.report_status == 0" @click="triggle(Report)" type="button">屏蔽</button>
                        <button v-else @click="triggle(Report)" type="button">解除</button>
                        <button @click="showConfirm(0, index, Report.share_report_id, Report.share_id)" type="button">删除</button>
                    </td>
                </tr>
            </template>
        </table>
    ...
    export default{
        ...
        methods: {
            ...
            getReports () {
                ...
                this.$http.get(this.$adminUrl('sharereport')).then((response) => {
             ...
             *  显示删除提示框
             */
            showConfirm (type, index=0, reportId=0, shareId=0) {
                this.type = type;
                if(type == 0){
                    this.show = true;
             ...
             *  删除单个分享分类
             */
            deleteReport (index, reportId, shareId) {

                this.$http.delete(this.$adminUrl('sharereport/') + reportId, {params:{shareId: shareId}}).then((response) => {
             ...
             */
            batchDelete () {
                this.$http.delete(this.$adminUrl('sharereport/batch-delete'),{params:this.checkIds})
                    .then((response) => {
            ...
            triggle (report) {
                this.$http.post(this.$adminUrl('sharereport/triggle'), {reportId: report.share_report_id, shareId: report.share_id}).then((response) => {
                    if(response.body <= 1){
            ...
            more (id) {
                this.$http.get(this.$adminUrl('sharereport/')+id).then((response) => {
